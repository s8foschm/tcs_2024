...
\begin{enumerate}
    \item[\textbf{(i)}] That is not True, a counter example is what we have seen in the last 2 Subproblems where $A_k := \{0^k1^k\}$ is finite and therefore regular but \( A := \bigcup_{k \in \mathbb{N}} A_k \) is not regular as shown in b).
